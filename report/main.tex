...
\noindent
The CNN (88M params) and the ViT (86M params) were fine-tuned on a new dataset $\mathcal{D}$ comprising unknots $\mathcal{U}$ and non-trivial knots $\mathcal{K}$
partitioned into three disjoint data splits $S = \{\mathcal{D}_{\mathrm{train}}, 
\mathcal{D}_{\mathrm{val}}, \mathcal{D}_{\mathrm{test}}\}$, where:\\

\[
\begin{gathered}
\mathcal{D} = \bigsqcup_{s \in S} s \\[0.5em]
...
\forall s \in S, \;\, s \in \; \mathcal{U} \, \cup \, \mathcal{K} \;\; \text{ and } \;\; |\mathcal{U}| = |\mathcal{K}|. \\[0.5em]
\end{gathered}
\]\\


\noindent
Each split in $\mathcal{D}$ contained knots with $n \in N := \{20, \,21, \, \ldots \,, \,35\}$ crossings.
For all distinct $a, b \in N$, each split in $S$ contained an equal number of $a$ and $b$-crossing unknot and non-trivial knot diagrams.
